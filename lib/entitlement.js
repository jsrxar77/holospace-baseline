...
async function getTenantSubscriptionAndUsage(tenantId) {
  ...
  const usersCountRow = await getOne('SELECT COUNT(*) as count FROM core_users WHERE tenant_id = ? AND is_active = true', [tenantId], { tenantId, isSuperAdmin: true });
  const ordersCountRow = await getOne(
    "SELECT COUNT(*) as count FROM kanban_orders WHERE tenant_id = ? AND created_at >= date_trunc('month', CURRENT_TIMESTAMP)",
    [tenantId],
    { tenantId, isSuperAdmin: true }
  );
