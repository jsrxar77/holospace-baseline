/**
 * lib/auth.js - HoloSpace Multi-Tenant Security, Hashing & JWT Auth Engine
...
const JWT_SECRET = process.env.JWT_SECRET || 'holospace_super_secret_jwt_key_2026_x89f_aes';
...
function getJwtSecret() {
  return process.env.JWT_SECRET || 'holospace_super_secret_jwt_key_2026_x89f_aes';
}
...
async function resolveTenantContext(req, currentUser = null) {
  ...
  // C. Subdominio (ej: drinklovers.holospace.app)
  ...
  let tenant = null;
  if (tenantId) {
    tenant = await getOne('SELECT * FROM tenant_tenants WHERE id = ?', [tenantId]);
  } else if (tenantSlug) {
    tenant = await getOne('SELECT * FROM tenant_tenants WHERE slug = ?', [tenantSlug]);
  }

  if (!tenant) {
    tenant = { id: DEFAULT_TENANT_ID, slug: DEFAULT_TENANT_SLUG, name: 'HoloSpace Cloud Platform', status: 'active' };
  }
