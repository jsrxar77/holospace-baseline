...
const PLANS = {
  ...
  pro: {
    ...
    description: 'Para centros de distribución y empresas de logística medianas con inteligencia e-commerce.'
  },
  enterprise: {
    code: 'enterprise',
    ...
    roleQuotas: { max_admins: 9999, max_operators: 9999, max_analysts: 9999 },
    description: 'Capacidad ilimitada, soporte 24/7 y todos los módulos desbloqueados.'
  },
  // Línea Vertical Logística (Kanban + Scanner)
  kanban_simple: {
    code: 'kanban_simple',
    name: 'Kanban Simple',
    priceUsd: 39,
    ...
    maxOrdersMonthly: 500,
    includedModules: ['core', 'kanban', 'scanner'],
    roleQuotas: { max_admins: 1, max_operators: 3, max_analysts: 0 },
    description: 'Para depósitos individuales o pequeñas operaciones.'
  },
  kanban_business: {
    code: 'kanban_business',
    name: 'Kanban Business',
    priceUsd: 119,
    maxUsers: 18,
    maxOrdersMonthly: 3000,
    includedModules: ['core', 'kanban', 'scanner'],
    roleQuotas: { max_admins: 3, max_operators: 15, max_analysts: 0 },
    description: 'Para centros de distribución con múltiples operarios y supervisores.'
  },
  kanban_enterprise: {
    code: 'kanban_enterprise',
    name: 'Kanban Enterprise',
    priceUsd: 299,
    maxUsers: 9999,
    maxOrdersMonthly: 999999,
    includedModules: ['core', 'kanban', 'scanner'],
    roleQuotas: { max_admins: 9999, max_operators: 9999, max_analysts: 0 },
    description: 'Capacidad masiva ilimitada para grandes redes logísticas.'
  },
  // Línea Vertical E-Commerce (4see Intelligence)
  fourseee_simple: {
    code: 'fourseee_simple',
    name: '4see Simple',
    priceUsd: 49,
    maxUsers: 3,
    maxOrdersMonthly: 0,
    includedModules: ['core', '4see'],
    roleQuotas: { max_admins: 1, max_operators: 0, max_analysts: 2 },
    description: 'Vigilancia de precios y márgenes para vendedores individuales.'
  },
  fourseee_business: {
    code: 'fourseee_business',
    name: '4see Business',
    priceUsd: 149,
    maxUsers: 10,
    maxOrdersMonthly: 0,
    includedModules: ['core', '4see'],
    roleQuotas: { max_admins: 2, max_operators: 0, max_analysts: 8 },
    description: 'Inteligencia de catálogo y repricing para tiendas en expansión.'
  },
  fourseee_enterprise: {
    code: 'fourseee_enterprise',
    name: '4see Enterprise',
    priceUsd: 349,
    maxUsers: 9999,
    maxOrdersMonthly: 0,
    includedModules: ['core', '4see'],
    roleQuotas: { max_admins: 9999, max_operators: 0, max_analysts: 9999 },
    description: 'Monitoreo multi-cuenta corporativo ilimitado.'
  }
};
...
async function registerNewTenant({ companyName, slug, adminName, adminEmail, password, planCode = 'starter' }) {
  ...
  // 1. Verificar  // 1. Validar slug único
  ...
  await execute(
    ...
    [userId, tenantId, cleanEmail.split('@')[0], cleanEmail, passwordHash, adminName.trim(), 'ADMIN'],
    { isSuperAdmin: true }
  );
...
async function createCheckoutSession(tenantId, targetPlanCode, successUrl = '/#subscription-success') {
  ...
  const tenant = await getOne('SELECT * FROM tenant_tenants WHERE id = ?', [tenantId], { tenantId, isSuperAdmin: true });
  if (!tenant) {
  ...
  return {
    success: true,
    sessionId,
    checkoutUrl: `https://checkout.holospace.app/pay/${sessionId}?tenant=${tenant.slug}&plan=${plan.code}`,
    plan: {
      code: plan.code,
      name: plan.name,
