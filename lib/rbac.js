...
async function getUserPermissions(user) {
  ...
  // Si posee role_id, consultar en role_permissions
  if (user.role_id) {
  ...
  try {
    const roleRow = await getOne(
      'SELECT id FROM roles WHERE LOWER(code) = ? AND (tenant_id IS NULL OR tenant_id = ?)',
...
async function getAllPermissions() {
  try {
    const rows = await query(
      'SELECT key, module_code, name, description, category FROM permissions ORDER BY module_code, key ASC',
      [],
      { isSuperAdmin: true }
    );
    return rows;
...
async function updateCustomRole(arg1, arg2, arg3, arg4) {
  ...
  const role = await getOne('SELECT * FROM roles WHERE id = ?', [roleId], { isSuperAdmin: true });
  ...
  if (Array.isArray(permissions)) {
    await execute('DELETE FROM role_permissions WHERE role_id = ?', [roleId], { isSuperAdmin: true });
    for (const permKey of permissions) {
      await execute(
        'INSERT INTO role_permissions (role_id, permission_key) VALUES (?, ?) ON CONFLICT DO NOTHING',
        [roleId, permKey],
...
async function deleteCustomRole(roleId, tenantId, isSuperAdmin = false) {
  ...
  await execute('DELETE FROM roles WHERE id = ?', [roleId], { isSuperAdmin: true });
